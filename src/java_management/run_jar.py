...
# Set source key
SOURCE_KEY = str(clr("[RUN_JAR] ", 'magenta'))
...
def runJar(dir, file_name):
    ...
    if not doesJarExist(jar_file):
        log(f"Jar '{clr(jar_file, 'yellow')}' could not be executed... Returning early")
        return
    ...
    try:
        # Run the Server
        log(f"Running jar '{clr(jar_file, 'yellow')}'")
    ...
    # Error if the server cannot be installed due to the sub process
        ...
        log(f"Could not run jar '{clr(jar_file, 'yellow')}'")
        log(f"Failed with exception {e.returncode}")



    log(f"Stopping jar '{clr(jar_file, 'yellow')}'")
    return

# Check if a jar exists
def doesJarExist(file_path):
    log(f"Checking if jar '{file_path}' exists")

    # Check for the jar path
    if os.path.exists(file_path):
        log(f"Jar '{clr(file_path, 'yellow')}' {clr('does', 'green')} exist")
        return True

    log(f"Jar '{clr(file_path, 'yellow')}' {clr('does not', 'red')} exist")
    return False
